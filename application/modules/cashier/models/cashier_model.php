...
class Cashier_model extends CI_Model 
{
          ...
          if(count($item_invoiced_rs) > 0){
            ...
            foreach ($item_invoiced_rs as $key_items):
			  ...
			  //If pharmacy
			  	if($service_id == 4)
				{
					if($this->cashier_model->in_pres($service_charge_id, $visit_id))
					{
	...
	{
	      $payments_rs = $this->cashier_model->payments($visit_id);
	      $total_payments = 0;
	...
	public function receipt($visit_id, $invoice = NULL)
	{
		...
		*/
		...
		$item_invoiced_rs = $this->cashier_model->get_patient_visit_charge_items($visit_id);
		$total = 0;
		$pageH = 6;
		$width = 22.5;
	...
	public function get_service_charge_list($table, $where, $order)
	{
		//retrieve all users
		$this->db->from($table);
		$this->db->select('*');
	...
	function get_visit_service_charges($v_id)
	{
	...
	public function submitorderservicecharge($service_charge_id,$order_id,$suck)
	{
		$visit_data = array('service_charge_id'=>$service_charge_id,'visit_id'=>$order_id,'units'=>$suck,'created_by'=>$this->session->userdata("personnel_id"),'created'=>date("Y-m-d"));
		$this->db->insert('visit_order', $visit_data);
	}
	function visit_charge_insert($order_id,$service_charge_id,$suck)
	{
		$service_charge_rs = $this->get_service_charge($service_charge_id);
	...
	function get_service_charge($service_charge_id){
		$table = "service_charge, service";
		$where = "service_charge.service_id = service.service_id AND service_charge.service_charge_id = '$service_charge_id'";
	...
	public function get_service_charges_per_type($service_id)
	{
		$table = "service_charge";
		$where = "service_id = $service_id and service_charge_status = 1";
		$items = "*";
		$order = "visit_type_id";
			//echo $sql;
		$result = $this->database->select_entries_where($table, $where, $items, $order);
