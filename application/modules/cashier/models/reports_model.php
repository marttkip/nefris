...
class Reports_model extends CI_Model 
{
	...
	*/
	...
	public function get_queue_total($date = NULL, $where = NULL)
	{
		...
		if($where == NULL)
		{
			$where = 'orders.order_id = visit_department.visit_id AND visit.close_card = 0 AND visit.visit_date = \''.$date.'\'';
		}
		
		else
		{
			$where .= ' AND orders.order_id = visit_department.visit_id AND visit.close_card = 0 AND visit.visit_date = \''.$date.'\' ';
		}
		
		$this->db->select('COUNT(orders.order_id) AS queue_total');
		$this->db->where($where);
	...
	*/
	...
	public function get_payment_method_total($payment_method_id, $date = NULL)
	{
		...
		$this->db->where('payments.visit_id = orders.order_id AND payment_method_id = '.$payment_method_id.' AND visit_date = \''.$date.'\'');
	...
	public function get_service_total($service_id, $date = NULL)
	{
		...
		{
			$where = 'visit_charge.service_charge_id = service_charge.service_charge_id AND service_charge.service_id = '.$service_id.' AND orders.order_id = visit_charge.visit_id'. $visit_search;
			$table .= ', visit';
		}
	...
	*/
	public function get_total_services_revenue($where, $table)
	{
		...
		$this->db->select('SUM(visit_charge.visit_charge_amount * visit_charge.visit_charge_units) AS total_invoiced');
		$this->db->where($where.' AND orders.order_id = visit_charge.visit_id AND visit_charge.service_charge_id = service_charge.service_charge_id AND visit_charge.visit_charge_delete =0');
		$query = $this->db->get();
	...
	*/
	public function get_total_cash_collection($where, $table, $page = NULL)
	{
		...
		if($page != 'cash')
		{
			$where .= ' AND orders.order_id = payments.visit_id AND payments.cancel = 0';
		}
		...
		if($visits_query->num_rows() > 0)
		{
			...
			*/
			...
			foreach($visits_query->result() as $row)
			{
				...
				$payments_value = $this->cashier_model->total_payments($visit_id);

				$invoice_total = $this->cashier_model->total_invoice($visit_id);
				...
				$credit_note_amount = $this->cashier_model->get_sum_credit_notes($visit_id);
				$debit_note_amount = $this->cashier_model->get_sum_debit_notes($visit_id);
				// end of total debit and credit notes amount

				// get all the payment methods used in payments
				//$payment_type = $this->cashier_model->get_visit_payment_method($visit_id);
	...
	*/
	public function get_visit_departments($where, $table)
	{
		...
		$this->db->where($where.' AND orders.order_id = visit_department.visit_id');
		$query = $this->db->get();
	...
	public function get_total_collected($doctor_id, $date_from = NULL, $date_to = NULL)
	{
		...
		$where = 'visit_charge.visit_id = orders.order_id AND visit.personnel_id = '.$doctor_id;
		
		$visit_search = $this->session->userdata('all_doctors_search');
		if(!empty($visit_search))
		{
			$where = 'visit_charge.visit_id = orders.order_id AND visit.personnel_id = '.$doctor_id.' '. $visit_search;
		}
	...
	*/
	function export_cash_report()
	{
		...
		$where = 'payments.payment_method_id = payment_method.payment_method_id AND payments.visit_id = orders.order_id AND payments.payment_type = 1 AND visit.visit_delete = 0 AND visit.branch_code = \''.$branch_code.'\' AND visit.patient_id = patients.patient_id AND visit_type.visit_type_id = visit.visit_type AND payments.cancel = 0';
	...
	{
		$this->db->select('SUM(visit_charge.visit_charge_units * visit_charge.visit_charge_amount) AS invoice_amount, patients.patient_surname, patients.patient_othernames, patients.patient_number, patients.current_patient_number, orders.order_id, visit.visit_date, visit.patient_insurance_number, debtor_invoice_item.debtor_invoice_item_status, debtor_invoice_item.debtor_invoice_item_id');
		$this->db->where('visit.visit_delete = 0 AND orders.order_id = debtor_invoice_item.visit_id AND visit.patient_id = patients.patient_id AND orders.order_id = visit_charge.visit_id AND debtor_invoice_item.debtor_invoice_id = '.$debtor_invoice_id);
