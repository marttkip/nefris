...
{	
	...
	public function all_transactions($module = '__')
	{
		...
		$where2 = $where;
		$v_data['orders'] = $this->reports_model->count_items($table, $where2);
	...
	public function search_transactions($module = '__')
	{
		...
		if($debtors == 'false')
		{
			$prev_search = ' AND payments.visit_id = orders.order_id AND payments.payment_type = 1';
			$prev_table = ', payments';
		...
		else
		{
			if(!empty($visit_date_from) && !empty($visit_date_to))
			{
				$visit_date = ' AND orders.orders_date BETWEEN \''.$visit_date_from.'\' AND \''.$visit_date_to.'\'';
				$search_title .= 'Orders date from '.date('jS M Y', strtotime($visit_date_from)).' to '.date('jS M Y', strtotime($visit_date_to)).' ';
			}
			
			else if(!empty($visit_date_from))
			{
				$visit_date = ' AND orders.orders_date = \''.$visit_date_from.'\'';
				$search_title .= 'Orders date of '.date('jS M Y', strtotime($visit_date_from)).' ';
			}
			
			else if(!empty($visit_date_to))
			{
				$visit_date = ' AND orders.orders_date = \''.$visit_date_to.'\'';
				$search_title .= 'Orders date of '.date('jS M Y', strtotime($visit_date_to)).' ';
			}
	...
	public function debtors_report_invoices($visit_type_id, $order = 'debtor_invoice_created', $order_method = 'DESC')
	{
		...
		if($visit_type_id > 0)
		{
			...
			$where .= ' AND debtor_invoice.debtor_invoice_id = debtor_invoice_item.debtor_invoice_id AND orders.order_id = debtor_invoice_item.visit_id ';
